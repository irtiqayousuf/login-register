...
export default function Basic(){
  ...
  const [value, setValue] = React.useState('v1');
    ...
    const formHandler = (e) => {
        ...
        const response = axios.post("http://localhost:8080/saveCustomer",{plan:plan, website : website, name : name , phone : phone, email: email });
    ...
    return(
     ...
     //</Box> <Box sx={{ display: 'flex' }}>
         ...
         <Paper elevation={24} style={paperStyle}  variant="outlined" >
                ...
                <Input  type='text' label='Plan'required placeholder='Basic' name='plan' value='Basic Plan' fullWidth />
                </Stack>
                  <Stack spacing={4} direction="row" paddingBottom={5}>
